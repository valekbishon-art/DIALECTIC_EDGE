...
async def _safe_send_message(chat_id: int, text: str, reply_markup=None):
    """Send as Markdown, but never let a malformed-entity error swallow the
    message. Model-generated digests sometimes contain unbalanced *, _, ` or [
    which Telegram's legacy Markdown rejects (TelegramBadRequest: can't parse
    entities ... offset N). On that specific failure we resend the SAME text as
    plain text (no parse mode) so the user always receives the content."""
    try:
        ...
        if "parse" not in str(e).lower() and "entit" not in str(e).lower():
            raise
        logger.warning("Markdown parse failed (%s) — resending as plain text", e)
        return await bot.send_message(
...
async def _run_daily_for_horizon(
    ...
    *,
    ...
    if not force_fresh and horizon_key in (DEFAULT_HORIZON_KEY, "mid", ""):
        try:
            from payments.db import get_today_digest
            pg_digest = await get_today_digest()
            if pg_digest and (pg_digest.get("short_report") or pg_digest.get("digest_text")):
    ...
    # Generation (run_full_analysis) is expensive and is meant to run ONCE per
    # day via cron_digest.py (08:50 MSK → cached in PG). Only an admin may force
    # an on-demand fresh build. For everyone else, if today's cached digest is
    # not ready yet we tell them when it lands instead of silently generating a
    # brand-new (and costly) one.
    if not force_fresh:
        if wait_msg_id is not None:
            try:
                await bot.delete_message(chat_id=chat_id, message_id=wait_msg_id)
        ...
        await _safe_send_message(
            chat_id,
            "🧠 *Свежий Дайджест Диалектики* выходит каждое утро в *09:00 МСК*.\n\n"
            "Сегодняшний ещё формируется — загляни чуть позже или открой "
            "🗂 *Базу Дайджестов*, чтобы посмотреть предыдущие.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🗂 База Дайджестов", callback_data="cmd:digest_base")],
            ]),
        )
        return
...
_DIALECTICA_INTRO = (
    ...
    "Каждое утро в *09:00 МСК* система готовит свежий *Дайджест Диалектики* на горизонт "
    "*7–14 дней*. Нажми кнопку ниже, чтобы получить сегодняшний, или открой базу "
...
def _dialectica_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📩 Получить Дайджест Диалектики (09:00 МСК)",
