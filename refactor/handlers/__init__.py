...
# [removed] funding_handler удалён — funding rate / carry = риба (харам).
...
# [removed] sniping_handler удалён — снайпинг листингов = майсир/спекуляция (харам).
...
# [removed] postmortem_handler удалён — пост-мортем ликвидационных каскадов (деривативы = харам).
...
# [removed] pump_handler удалён — памп-сканер (спекуляция/шорт-фейд = харам).
...
# ─────────────────────────────────────────────────────────────────────────────
# [removed] Заглушки-no-op для удалённых ХАРАМ-хендлеров. main.py импортирует и
# вызывает эти имена при старте; оставляем их как безопасные пустышки, чтобы
# импорт-граф и запуск не падали, а сами харам-фичи (funding/sniping/pump/
...
def register_sniping_handlers(dp=None, *args, **kwargs) -> None:
    ...
    _logger.info("[removed] sniping/pump handlers отключены (харам).")


def register_funding_handlers(dp=None, *args, **kwargs) -> None:
    """[removed] funding carry удалён — no-op."""
    _logger.info("[removed] funding handlers отключены (риба).")
...
async def handle_funding_command(*args, **kwargs) -> None:
    """[removed] /funding удалён (риба) — no-op."""
    _logger.info("[removed] /funding вызван, но фича удалена (риба).")
