"""
halal_edge_backtest.py — честный бэктест спот-EDGE на ПОЛНОМ цикле.
...
from halal_edge import (  # noqa: E402
    UNIVERSE, CORE4, FEE, SMA_BTC, DEFAULT_CFG, ENHANCED_CFG,
...
def load_data() -> tuple[list[str], dict[str, list[float | None]]]:
    # Офлайн-режим: если рядом лежит prices_cache.json (скачанный
    # research/download_prices.py на машине с интернетом) — берём РЕАЛЬНЫЕ
    # цены из него, без обращения к сети.
    cache = ROOT / "prices_cache.json"
    if cache.exists():
        obj = json.loads(cache.read_text(encoding="utf-8"))
        days = obj["days"]
        series = {s: obj["series"][s] for s in UNIVERSE if s in obj.get("series", {})}
        if days and series.get("BTC"):
            print(
                f"Беру РЕАЛЬНЫЕ цены из {cache.name} (офлайн): "
                f"{len(days)} дней, {len(series)} монет ({days[0]}→{days[-1]})."
            )
            return days, series
        print(f"{cache.name} пустой/битый — качаю с Yahoo…")
...
# ───────────────────────── robustness ─────────────────────────
def robustness(days, series, base_cfg=None) -> dict:
    ...
    base_cfg = base_cfg or DEFAULT_CFG
    grid = []
    for sma_t in (100, 120, 150):
        for k in (3, 4, 5):
            for wm in ("mom", "equal"):
                cfg = dict(base_cfg, sma_trend=sma_t, top_k=k, weight_mode=wm)
...
def run_all() -> dict:
    ...
    enhanced = run_edge(days, series, ENHANCED_CFG)
    ...
    rob_enh = robustness(days, series, base_cfg=ENHANCED_CFG)
    ...
    metrics = {
        ...
        "enh_total": enhanced["total"], "enh_cagr": enhanced["cagr"],
        "enh_mdd": enhanced["mdd"], "enh_sharpe": enhanced["sharpe"], "enh_exposure": enhanced["exposure"],
        "rob": rob, "rob_enh": rob_enh,
    }
    return {
        "metrics": metrics,
        "curves": {
...
def render_chart(res: dict) -> Path:
    ...
    enh = c.get("enhanced", c["edge"])[1:]
    n = min(len(days), len(edge), len(btc), len(basket), len(base), len(enh))
    days, edge, btc, basket, base, enh = days[:n], edge[:n], btc[:n], basket[:n], base[:n], enh[:n]
    ...
    ax.plot(days, [v * 100 for v in enh], color="#a855f7", lw=2.0, label="EDGE V2 (медленный импульс + концентрация)")
...
def render_md(res: dict) -> Path:
    m = res["metrics"]
    r = m["rob"]
    re = m.get("rob_enh") or r
    p = lambda x: f"{x * 100:+.1f}%"
    _enh_better = (m.get("enh_sharpe", 0) >= m["strat_sharpe"]) and (re["sharpe_med"] >= r["sharpe_med"])
    verdict = (
        "EDGE V2 улучшает риск-метрики и на полном цикле, И по сетке — есть смысл включить в прод."
        if _enh_better else
        "EDGE V2 НЕ даёт устойчивого улучшения на этом периоде — остаёмся на базовом EDGE."
    )
    now = datetime.now(tz=timezone.utc).strftime("%Y-%m-%d")
    lines = [
        "# 📊 Бэктест EDGE-стратегии — Dialectic Edge (полный цикл)",
        ...
        "",
        "## 🆕 A/B: базовый EDGE vs EDGE V2 (подтверждённые перебором фишки)",
        "",
        "**EDGE V2 (сбалансированный)** = те же веса/юниверс, но: более длинные окна импульса (60/120/240) "
        "и более сильная концентрация веса на топ-импульсе (mom_power=1.5). Подтверждено на реальных данных.",
        "",
        "| Метрика | EDGE (база) | 🆕 EDGE V2 |",
        "|---|---|---|",
        f"| Доходность | {p(m['strat_total'])} | **{p(m['enh_total'])}** |",
        f"| CAGR | {p(m['strat_cagr'])} | **{p(m['enh_cagr'])}** |",
        f"| Макс. просадка | {p(m['strat_mdd'])} | **{p(m['enh_mdd'])}** |",
        f"| Sharpe | {m['strat_sharpe']:.2f} | **{m['enh_sharpe']:.2f}** |",
        f"| Время в рынке | {p(m['exposure'])} | {p(m['enh_exposure'])} |",
        f"| Робастность: Sharpe (медиана сетки) | {r['sharpe_med']:.2f} | **{re['sharpe_med']:.2f}** |",
        f"| Робастность: CAGR (медиана сетки) | {p(r['cagr_med'])} | **{p(re['cagr_med'])}** |",
        "",
        f"_Вердикт: {verdict}_",
        "",
        "## Что это значит",
        "",
...
def render_json(res: dict) -> Path:
    ...
    rob_enh = m.pop("rob_enh", None)
    if rob_enh:
        m["rob_enh_cagr_med"] = rob_enh["cagr_med"]
        m["rob_enh_mdd_med"] = rob_enh["mdd_med"]
        m["rob_enh_sharpe_med"] = rob_enh["sharpe_med"]
    DOCS.mkdir(exist_ok=True)
...
def main():
    res = run_all()
    md = render_md(res)
    render_json(res)            # цифры сохраняем ДО графика
    try:
        png = render_chart(res)
    except ModuleNotFoundError:
        png = "(пропущен: нет matplotlib — pip install matplotlib)"
        print("\n⚠️ matplotlib не установлен — график пропущен, "
              "но цифры в docs/BACKTEST_RESULTS.md и docs/backtest_summary.json готовы.")
    m = res["metrics"]
    r = m["rob"]
    ...
    print(f"EDGE+    : {pp(m['enh_total'])}  CAGR {pp(m['enh_cagr'])}  "
          f"MDD {m['enh_mdd']*100:.1f}%  Sharpe {m['enh_sharpe']:.2f}  expo {m['enh_exposure']*100:.0f}%")
